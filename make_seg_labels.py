...
def _work(process_id, model, dataset, args):
    ...
    if os.path.isdir(args.session_name + f'/npy_trainaug') :
        cam_out_dir = os.path.join(args.session_name + f'/npy_trainaug')
    else :
        cam_out_dir = os.path.join(args.session_name, 'npy')
    ...
    with torch.no_grad(), cuda.device(process_id):
        ...
        for iter, pack in enumerate(data_loader):
            ...
            imageio.imsave(os.path.join(f"{args.session_name}/{args.sem_seg_out_dir}", img_name + '.png'), rw_pred.astype(np.uint8))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--sem_seg_out_dir", default="pseudo_label", type=str)
    ...
    os.makedirs(f"{args.session_name}/{args.sem_seg_out_dir}", exist_ok=True)
