...
def _work(process_id, model, dataset, args):
    save_path = 'Results_agglomerativeclustering'
    os.makedirs(os.path.join(save_path), exist_ok=True)

    # save_path_is_cam = f'{save_path}/iscam'
    # os.makedirs(os.path.join(save_path_is_cam), exist_ok=True)

    save_path_seed = f'{save_path}/seed'
    os.makedirs(os.path.join(save_path_seed), exist_ok=True)
    ...
    with torch.no_grad(), cuda.device(process_id):
        ...
        for iter, pack in enumerate(data_loader):
            ...
            if args.single_img :
                ...
                # _, _, _, updated_cam = model(img, label, denorm_img)      
                clustering_seeds = model(img, label, denorm_img)      

                orig_img = np.array(Image.open(f"{image_path}/{img_name}.jpg").convert('RGB'))        

                clustering_seeds.reverse()

                # for i in range(len(updated_cam)) :
                for i in range(len(clustering_seeds)) :
                    # draw seed
                    save_path_i = f"{save_path_seed}/{i}"
                    os.makedirs(os.path.join(save_path_i), exist_ok=True)
                    
                    seed = clustering_seeds[i]               
                    draw_segmap(VOC_color[seed], orig_img, save_path_i, pack['name'][0])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--network", default="network.resnet50_SIPE_refined2_1_norm_iscam2_agglomerative_clustering", type=str)
